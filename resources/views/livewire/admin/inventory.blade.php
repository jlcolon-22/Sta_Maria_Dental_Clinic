<div class="bg-[#f2f6fa] max-h-[100svh]  ">
    ...
    <x-admin.aside>
        <section x-data="{add:false}" x-init="() => {
            Livewire.on('somethingUpdated', () => {
               add = false
            })
        }"
            ...
            <div class="bg-white shadow-md rounded-md mt-7  p-5  ">
                ...
                <div class="mt-5 py-2 flex items-center justify-between">
                    <button class="py-2 px-4 bg-ylw text-white rounded" x-on:click="add = true">Add Product</button>
                    <form action="" class="relative w-fit">
                        <input type="tel" autocomplete="off"
                        class="border py-2 pl-2 pr-[3.1rem] focus:border-ylw outline-none bg-gray-100 "
                        placeholder="Search">
                        <button class="absolute h-full w-[3rem] flex justify-center items-center top-0 right-0  bg-primary hover:opacity-75">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-[2rem] text-white"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                        </button>
                    </form>
                        ...
                        <tbody class="text-gray-900">
                            <tr>
                                ...
                                <td class="py-3 text-center px-2 flex justify-center gap-x-2">
                                    <button class="text-yellow-500 font-robotoBold hover:underline">Update</button>

                                    |
                                    <button class="text-red-500 font-robotoBold hover:underline">Delete</button>
                                </td>
                            </tr>
            ...
            {{-- add modal --}}
            <div  x-show="add" class="fixed top-0 w-full min-h-[100svh] max-h-[100svh] flex justify-center overflow-y-auto bg-black/10 left-0 pt-20">
                    <form wire:submit.prevent="store" method="POST" x-show="add"  x-transition class="bg-white shadow-md border w-[30rem] h-fit p-3 rounded-md">
                       <div class="flex justify-between items-center">
                        <h1 class="font-robotoBold text-xl">Add Product</h1>
                        <button type="button"  x-on:click="add = false">
                        ...
                        <div class="grid mt-6 ">
                            <label for="" class="font-medium text-gray-800">Product Name<span class="text-red-600">*</span></label>
                            <input type="text" autocomplete="off" wire:model="data.name"
                            class="border py-2 pl-2 pr-[3.1rem] focus:border-ylw outline-none bg-gray-100 "
                            placeholder="ex: BETADINE">
                        </div>
                        <div class="grid mt-2 ">
                            <label for="" class="font-medium text-gray-800">Quantity<span class="text-red-600">*</span></label>
                            <input type="number" min="1" value="1" wire:model="data.quantity" autocomplete="off"
                            class="border py-2 pl-2 pr-[3.1rem] focus:border-ylw outline-none bg-gray-100 "
                            placeholder="1">
                        </div>
                        <div class="grid mt-4 ">
                           <button type="submit" class="border py-2  focus:border-ylw outline-none bg-ylw text-white ">
